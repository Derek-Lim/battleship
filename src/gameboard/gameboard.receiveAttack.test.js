...
describe('receiveAttack', () => {
  ...
  describe('happy path', () => {
    ...
    describe('returns status object upon attack', () => {
      it('returns { hit: true, sunk: true}', () => {
        const ship = Ship(1)
        const orientation = 'horizontal'

        board.placeShip(ship, {x, y}, orientation)
        expect(board.receiveAttack(x, y)).toStrictEqual({ hit: true, sunk: true})
      })

      it('returns { hit: true, sunk: false}', () => {
        const ship = Ship(2)
        const orientation = 'horizontal'

        board.placeShip(ship, {x, y}, orientation)
        expect(board.receiveAttack(x, y)).toStrictEqual({ hit: true, sunk: false})
      })

      it('returns { hit: false, sunk: false}', () => {
        const ship = Ship(1)
        const orientation = 'horizontal'

        board.placeShip(ship, {x, y}, orientation)
        expect(board.receiveAttack(x, y + 1)).toStrictEqual({ hit: false, sunk: false})
        expect(board.receiveAttack(x + 1, y)).toStrictEqual({ hit: false, sunk: false})
      })

      it('returns exactly {hit:boolean, sunk:boolean}', () => {
        const ship = Ship(1)
        const orientation = 'vertical'

        board.placeShip(ship, {x, y}, orientation)
        const obj = board.receiveAttack(x, y)
        expect(Object.keys(obj).sort()).toStrictEqual(['hit', 'sunk'])
      })
    })
  })
  ...
  describe('unique cases', () => {
    it('still succeeds after an out-of-bounds attempt', () => {
      ...
      expect(result).toStrictEqual({ hit: true, sunk: true })
    })
