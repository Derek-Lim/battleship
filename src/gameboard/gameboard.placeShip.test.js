  ...
  describe('invalid placement', () => {
    ...
    describe('partial ship placement', () => {
      it('does not partially place on OOB failure', () => {
        ...
        expect(place).toThrow(Error)
        expect(place).toThrow(PLACESHIP_PLACEMENT_ERROR)
        expect(board.receiveAttack(x, y)).toStrictEqual({ hit: false, sunk: false })
      })
      ...
      it('does not partially place on overlap failure', () => {
        ...
        expect(place).toThrow(Error)
        expect(place).toThrow(PLACESHIP_PLACEMENT_ERROR)
        expect(board.receiveAttack(0, 0)).toStrictEqual({ hit: false, sunk: false })
        expect(board.receiveAttack(1, 0)).toStrictEqual({ hit: true, sunk: true })
        expect(board.receiveAttack(2, 0)).toStrictEqual({ hit: false, sunk: false })
      })
    })
  })
  ...
  describe('happy path', () => {
    ...
    it.each([1, 2, 3, 4])('places horizontal boat of length:%i', len => {
      ...
      for (let i = 0; i < len - 1; i++) {
        expect(board.receiveAttack(x + i, y)).toStrictEqual({ hit: true, sunk: false})
      }
      expect(board.receiveAttack(x + len - 1, y)).toStrictEqual({ hit: true, sunk: true})
    })
    it.each([1, 2, 3, 4])('places vertical boat of length:%i', len => {
      ...
      for (let i = 0; i < len - 1; i++) {
        expect(board.receiveAttack(x, y + i)).toStrictEqual({ hit: true, sunk: false})
      }
      expect(board.receiveAttack(x, y + len - 1)).toStrictEqual({ hit: true, sunk: true})
    })
  })
})
